...
state(Ex1,Ey1,Ec1,Agentsleft1,Agents,result(A,S)):-
	( (A=right, Ey is(Ey1+1), in_grid(Ex1,Ey), state(Ex1, Ey, Ec1, Agentsleft1,Agents, S));
	  ...
	  (A=down,  Ex is(Ex1+1), in_grid(Ex,Ey1), state(Ex, Ey1, Ec1, Agentsleft1,Agents, S));
...
reverse2(L, R) :-
	reverse2_helper(L, s0, R).

reverse2_helper(s0, R, R).
reverse2_helper(result(A,S), Acc, R) :-
	NewAcc = result(A,Acc),
	reverse2_helper(S, NewAcc, R).
